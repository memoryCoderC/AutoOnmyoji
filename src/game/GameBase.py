...
from src.image import ImageSearch, Image
from src.image.ImageSearch import best_match
...
class BaseOperator:
    ...
    def click_img(self, template_img_path):
        """
        点击图片
    ...
    def wait_img(self, img_path, max_time=30 * 1000):
        """
        等待游戏图像并点击
            :param max_time:
            :param self:
    ...
    def wait_img_click(self, img_path, max_time=30 * 1000):
        """
        等待游戏图像
            :param max_time:
            :param self:
    ...
    def screenshot_find(self, template_img_path):
        ...
        template_img = ImageSearch.read_img(template_img_path, 0)
    ...
    def random_pos(self):
        _left, _top, _right, _bot = self.window_size()
        x = random.randint(_left, _right)
        y = random.randint(_top, _bot)
        return [x, y]
